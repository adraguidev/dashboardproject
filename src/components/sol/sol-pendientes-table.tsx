...
interface SolPendientesTableProps {
...
export function SolPendientesTable({
  data,
  periodos,
  loading = false,
  className = '',
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet(`SOL Pendientes - ${groupBy}`);
    ...
    a.href = url;
    a.download = `sol-pendientes-${groupBy}.xlsx`;
  ...
  if (!data || data.length === 0) {
    return (
      <div className="p-10 text-center text-gray-500">
        No se encontraron datos de pendientes SOL en la hoja de cálculo.
      </div>
    )
  }
    ...
    <div className={className}>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6">
        <div className="flex items-center mb-4 lg:mb-0">
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-800">Reporte de Pendientes (SOL)</h3>
            <p className="text-sm text-gray-500">Expedientes SOL filtrados por estado, agrupados por evaluador y período.</p>
          </div>
        </div>
