...
const fetchSolProduccion = async (): Promise<ProduccionResponse> => {
  try {
    const response = await fetch('/api/sol/produccion')
    if (!response.ok) {
    ...
    logError('Error al obtener datos de producción SOL:', error)
    throw error
  }
}

export const useSolProduccion = () => {
  return useQuery<ProduccionResponse, Error>({
    queryKey: ['solProduccion'],
    queryFn: fetchSolProduccion,
