...
const fetchSolAsignaciones = async (days: number): Promise<AsignacionesResponse> => {
  try {
    const response = await fetch(`/api/sol/asignaciones?days=${days}`)
    if (!response.ok) {
    ...
    logError('Error al obtener datos de asignaciones SOL:', error)
    throw error
  }
}

export const useSolAsignaciones = (days: number) => {
  return useQuery<AsignacionesResponse, Error>({
    queryKey: ['solAsignaciones', days],
    queryFn: () => fetchSolAsignaciones(days),
